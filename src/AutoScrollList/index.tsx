...
function ReactAutoScrollList<Item>(props: IReactAutoScrollList<Item>) {
  const { data, timeInterval = 1000, renderItem, keyExtractor, className } = props;
  const [activeItemIndex, setActiveItemIndex] = React.useState<number>(0);
  const [interval, setInterval] = useState<number | null>(timeInterval);
  const containerRef = useRef<HTMLDivElement>(null);
  const containerSize = useSize(containerRef);
  const containerClassName = classnames('container', className);
  ...
  useEffect(() => {
    ...
    if (containerSize?.height && currentDom?.offsetHeight) {
      const itemCount = Math.floor(containerSize.height / currentDom.offsetHeight);
